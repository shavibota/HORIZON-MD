...
if (pairingCode && !conn.authState.creds.registered) {
  ...
  if (!!global.pairingNumber) {
    ...
    if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
      console.log(
        chalk.bgBlack(chalk.redBright("Start with your country's WhatsApp code, Example : 947xxxx"))
      )
      process.exit(0)
    }
  } else {
...
async function connectionUpdate(update) {
  ...
  if (code && code == DisconnectReason.restartRequired) {
    conn.logger.info(chalk.yellow('\RRestarting....'))
    process.send('reset')
  }
  ...
  if (connection === 'open') {
    ...
    let msgf = `Hey ${name} ... Congrats you have successfully deployed\n\nExperience seamless messaging with the HORIZON-MD WhatsApp bot, your multi-device companion. Effortlessly deployed, it connects you to a world of features and convenience. Scan, set up, and enjoy a richer chat experience\n\n\nJoin support Group : \n https://chat.whatsapp.com/KIpGJ7Z01T85Uq7NrEJu9q`
...
let isInit = true
let handler = await import('./ind_msg_con.js')
global.reloadHandler = async function (restatConn) {
  try {
    const Handler = await import(`./ind_msg_con.js?update=${Date.now()}`).catch(console.error)
